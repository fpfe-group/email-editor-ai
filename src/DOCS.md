# Mesagoo Email Editor — 完整技术文档

> **本文档目的**：为维护者提供修改、调试或扩展编辑器时需要了解的细节，
> 帮助在不引入回归的前提下开展工作。这里记录了架构决策、数据流、
> 不变量和容易忽略的细节。修改任何文件前，请先阅读本文档。

---

## 目录

1. [整体架构](#1-high-level-architecture)
2. [数据模型](#2-data-model)
3. [渲染管线](#3-rendering-pipeline)
4. [文件地图](#4-file-map)
5. [类型系统 (types.ts)](#5-type-system)
6. [Composables — 状态管理](#6-composables)
7. [序列化器](#7-serializers)
8. [Utilities](#8-utilities)
9. [Injection Keys 与 Provide/Inject 连接](#9-injection-keys)
10. [组件](#10-components)
11. [区块与入门模板](#11-blocks--starter-templates)
12. [属性系统](#12-properties-system)
13. [Iframe 桥接协议](#13-iframe-bridge-protocol)
14. [拖拽系统](#14-drag--drop-system)
15. [行内文本编辑](#15-inline-text-editing)
16. [撤销 / 重做](#16-undo--redo)
17. [源码视图 (CodeMirror)](#17-code-view)
18. [从 GrapesJS 迁移](#18-migration-from-grapesjs)
19. [CSS 约定](#19-css-conventions)
20. [绝不能破坏的不变量与规则](#20-invariants--rules)
21. [已知限制](#21-known-limitations)
22. [常见维护场景](#22-common-maintenance-scenarios)
23. [AI 模板生成](#23-ai-template-generation)
...
**核心原则**：编辑器围绕一棵映射 MJML 结构的 **JSON 文档树**工作。
每个用户操作都会修改这棵树。随后文档树会被序列化为 MJML 字符串，
通过 `mjml-browser` 编译为 HTML，并渲染到隔离的 `<iframe>` 中。
iframe 使用 `postMessage` 将点击、悬停和拖拽事件传回 Vue 层。

**没有全局 store**。所有状态都通过 composables + `provide/inject` 绑定在
`EmailEditor.vue` 实例范围内。这意味着同一页面上的多个编辑器实例会各自拥有独立状态。
