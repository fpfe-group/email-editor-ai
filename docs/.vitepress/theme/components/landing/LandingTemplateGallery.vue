...
const copyLabel = ref("复制 HTML");

// Categories matching the Pencil design pills — keys are manifest category values
const categories = [
  { key: "all", label: "全部" },
  { key: "marketing", label: "营销" },
  { key: "engagement", label: "用户互动" },
  { key: "transactionnel", label: "事务通知" },
  { key: "evenement", label: "活动" },
  { key: "industrie", label: "行业" },
];
...
onMounted(async () => {
  try {
    const resp = await fetch(
      import.meta.env.BASE_URL + "templates/manifest.json"
    );
...
const canPrev = computed(() => {
  ...
  return (
    displayedTemplates.value.findIndex(
...
const canNext = computed(() => {
  if (!editingTemplate.value) return false;
  const idx = displayedTemplates.value.findIndex(
    (t) => t.id === editingTemplate.value!.id
  );
  ...
  <section id="templates" class="gallery">
    ...
    <div class="gallery__header">
      <div class="gallery__header-top">
        <span class="gallery__eyebrow">入门模板</span>
      ...
      <h2 class="gallery__title">使用现成模板更快上线</h2>
      <p class="gallery__subtitle">
        所有模板都随 npm 包一起提供，无需账号，也无需额外 API 调用。
        导入后即可在编辑器中定制，并导出可用于生产的 HTML。
      </p>
      <div class="gallery__install-hint">
        <code class="gallery__install-code"
          >import { starterTemplates } from '@fpfe-group/email-editor-ai'</code
        >
      </div>
    </div>
    ...
    <div class="gallery__filters">
      ...
      >
        {{ cat.label }}
        <span v-if="categoryCounts[cat.key]" class="gallery__pill-count">{{
      ...
      >
        ...
        <template v-if="tpl.id === 'blank'">
          <div class="gallery__card-blank">
            ...
            <span class="gallery__card-blank-text">空白模板</span>
          </div>
        </template>
        <!-- Iframe preview card -->
        <template v-else>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="`编辑：${editingTemplate.label}`"
        >
          ...
          <div class="gallery__modal-controls-left">
            ...
            <span
              class="gallery__modal-badge"
              :style="{ background: editingTemplate.color }"
            />
          ...
          <div class="gallery__modal-controls-right">
            <div class="gallery__modal-nav">
              <button
                class="gallery__modal-nav-btn"
                :disabled="!canPrev"
                @click="navigateEditor(-1)"
                aria-label="上一个模板"
              >
                <svg
                  width="16"
...
.gallery__modal-editor
  :deep(.ebb-toolbar__group > .ebb-toolbar__action-btn:last-child),
.gallery__modal-editor
  :deep(.ebb-toolbar__group > .ebb-toolbar__divider:last-of-type) {
